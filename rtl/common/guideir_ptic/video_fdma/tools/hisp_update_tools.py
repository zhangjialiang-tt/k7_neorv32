...
def test():
    ...
    if response.status_code == 200:
        ...
        #         print(url, filename)
        # 更新Makefile脚本
        url = "https://gitee.com/i540113104/zg_fpga/raw/master/sim/sub0-grad/makefile"
        filename = "makefile"
        download_file_from_github(url, filename)
        # 更新configuration.json
        url = "http://192.168.100.181/guideir/Embedded_Group_Repositories/raw/master/Scripts/python/load_camera.py"
        filename = "test.py"
        download_file_from_github(url, filename)
    else:
        print("获取文件列表失败，状态码：", response.status_code)
