...
int test_sector_erase(void) {
    ...
    if (result == 0) {
        ...
        result = wait_for_flash_ready();
        if (result != 0) {
            print_test_result("Sector Erase - Wait for Ready", result);
            return result;
        }
...
int test_block_erase(void) {
    ...
    if (result == 0) {
        ...
        // 验证扇区是否已擦除
        result = verify_sector_erased(TEST_FLASH_ADDR, 16); // 验证前16个字节即可
...
int main() {
    ...
    total_result |= test_block_erase();      // <-- 使用新的调用
