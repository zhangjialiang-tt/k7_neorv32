...
/**************************************************************************
 * @file spi_flash_hal_test.c
 * @brief SPI Flash HAL test program.
 * @author NEORV32 User
 **************************************************************************/
...
/** Test flash address - changed to avoid potential protected sectors */
#define TEST_FLASH_ADDR 0x00A00000 // Changed from 0x00100000
/**@}*/
...
/**************************************************************************
 * @brief Verify sector is erased (all 0xFF)
 **************************************************************************/
int verify_sector_erased(uint32_t address, uint32_t size)
{
    uint8_t verify_buffer[TEST_BUFFER_SIZE];
    int result = spi_flash_hal_read(address, verify_buffer, size);

    if (result != 0)
    {
        neorv32_uart0_printf("Failed to read sector for verification\n");
        return -1;
    }
    ...
    for (uint32_t i = 0; i < size; i++)
    {
        ...
        {
            neorv32_uart0_printf("Erase verification failed at byte %d: expected 0xFF, got 0x%x\n",
                                 i, verify_buffer[i]);
            return -1;
        }
    }

    neorv32_uart0_printf("Sector erase verification PASSED\n");
    return 0;
}
...
int wait_for_flash_ready(void)
{
    ...
    do
    {
        ...
        if (result != 0)
        {
            neorv32_uart0_printf("Failed to read status\n");
            return -1;
        }
        ...
        if (--timeout == 0)
        {
            neorv32_uart0_printf("Flash operation timeout!\n");
            return -1;
        }
    } while (1);
}
...
/**************************************************************************
 * @brief Check flash protection status
 **************************************************************************/
int check_flash_protection(void)
{
    spi_flash_status_t status;
    int result = spi_flash_hal_read_status(&status);
    ...
    if (result != 0)
    {
        neorv32_uart0_printf("Failed to read protection status\n");
        return -1;
    }

    neorv32_uart0_printf("Flash protection status: WE=%d, BP=%d\n",
                         status.write_enabled, status.block_protect);
...
/**************************************************************************
 * @brief Test reading status register
 **************************************************************************/
int test_read_status(void)
{
    ...
    if (result == 0)
    {
        ...
        neorv32_uart0_printf("Block Protect: %d\n", status.block_protect);
    }
...
int test_write_enable_disable(void)
{
    ...
    if (result == 0)
    {
        ...
        neorv32_uart0_printf("Write Enabled Status: %d\n", status.write_enabled);
    ...
    if (disable_result == 0)
    {
        ...
        for (volatile int i = 0; i < 1000; i++)
            ;
        spi_flash_hal_read_status(&status);
        neorv32_uart0_printf("Write Disabled Status: %d\n", status.write_enabled);
        if (status.write_enabled)
        {
            disable_result = -1;
        }
...
/**************************************************************************
 * @brief Test sector erase
 **************************************************************************/
int test_sector_erase(void)
{
    neorv32_uart0_printf("Testing sector erase at address 0x%x...\n", TEST_FLASH_ADDR);

    int result = spi_flash_hal_erase_sector(TEST_FLASH_ADDR);

    if (result == 0)
    {
        // HAL 内部已等待完成并检查错误位（11.6.1）

        // Verify sector is erased (all 0xFF)
        result = verify_sector_erased(TEST_FLASH_ADDR, TEST_BUFFER_SIZE);
        print_test_result("Sector Erase - Verification", result);
    }
    else
    {
...
// spi_flash_hal_test.c

/**************************************************************************
 * @brief Test block erase (replaces sector erase test)
 **************************************************************************/
int test_block_erase(void)
{
    uint32_t erase_size = 64 * 1024; // 使用 64KB 块擦除
    neorv32_uart0_printf("Testing 64KB block erase at address 0x%x...\n", TEST_FLASH_ADDR);

    // 调用正确的块擦除函数
    int result = spi_flash_hal_erase_block(TEST_FLASH_ADDR, erase_size);

    if (result == 0)
    {
        // HAL函数已经是阻塞的，所以不需要外部等待
        neorv32_uart0_printf("Block erase command sent and completed.\n");
        // 验证扇区是否已擦除
        result = verify_sector_erased(TEST_FLASH_ADDR, 16);
        print_test_result("Block Erase - Verification", result);
    }
    else
    {
        print_test_result("Block Erase - Command Failed", result);
    }
...
{
    neorv32_uart0_printf("Testing data programming at address 0x%x...\n", TEST_FLASH_ADDR);

    // Check flash protection before programming
    int result = check_flash_protection();
    if (result != 0)
    {
        print_test_result("Data Program - Protection Check", result);
        return result;
    }
    ...
    if (result != 0)
    {
        print_test_result("Data Program - Write Enable", result);
        return result;
    }

    // Program the data（页编程，11.5）
...
int test_read(void)
{
    neorv32_uart0_printf("Testing data read from address 0x%x...\n", TEST_FLASH_ADDR);
