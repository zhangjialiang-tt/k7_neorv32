...
if {$use_elf} {
    ...
    set cmd "write_cfgmem -format $format -interface $interface -size $memory_size -loadbit {up 0x0 $bitstream_file} -loaddata {up 0x0800000 $elf_file} -file $output_file -force"
} else {
    set cmd "write_cfgmem -format $format -interface $interface -size $memory_size -loadbit {up 0x0 $bitstream_file} -checksum -disablebitswap -file $output_file -force"
}
# if {$force_bool} {
#     append cmd " -force"
# }
